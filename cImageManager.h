...
public:
	...
	void Render(cTexture * texture, Vec2 pos, float rot = 0, bool center = false);
	void Render(cTexture * texture, Vec2 pos, Vec3 size, float rot = 0, D3DCOLOR color = D3DCOLOR_XRGB(255, 255, 255), bool center = false);
	void CropRender(cTexture * texture, Vec2 pos, RECT rcRange);
	void CenterRender(cTexture * texture, Vec2 pos, const Vec3 & center, float rot = 0);
};

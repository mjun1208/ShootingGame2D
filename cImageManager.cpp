...
void cImageManager::PrintText(const string & str, Vec2 pos, float size, D3DCOLOR color, bool center)
{
	...
	if (!center)
	{
		...
		D3DXMatrixTranslation(&mat, pos.x - size * (str.size() * 0.2), pos.y - size / 2.f, 0);
	}
...
void cImageManager::Render(cTexture * texture, Vec2 pos, float rot, bool center)
{
	D3DXMATRIX mat;
...
void cImageManager::Render(cTexture * texture, Vec2 pos, Vec3 size, float rot, D3DCOLOR color, bool center)
{
	D3DXMATRIX mat;
	D3DXMATRIX matS, matR, matT;

	D3DXMatrixScaling(&matS, size.x, size.y, size.x);
	D3DXMatrixRotationZ(&matR, D3DXToRadian(rot));
	D3DXMatrixTranslation(&matT, pos.x, pos.y, 0);
...
void cImageManager::CropRender(cTexture * texture, Vec2 pos, RECT rcRange)
{
	D3DXMATRIX mat;
	D3DXMATRIX matS, matR, matT;
...
void cImageManager::CenterRender(cTexture * texture, Vec2 pos, const Vec3 & center, float rot)
{
	D3DXMATRIX mat;
	D3DXMATRIX matS, matR, matT;
